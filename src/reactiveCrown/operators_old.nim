...
proc publish*[T](upstream: Observable[T]): ConnectableObservable[T] =
  runnableExamples:
    ...
    var disconnectable = published.connect()
    ...
    consume disconnectable

    sbj.next()
    assert cntCalled == 2

  result = ConnectableObservable[T](
    subject: newOperatorSubject[T](),
    upstream: upstream,
    cacheDisp: cast[ptr Disposable](alloc sizeof Disposable)
  )
  zeroMem(result.cacheDisp, sizeof Disposable)
...
proc connect*[T](self: ConnectableObservable[T]): Disposable {.discardable.} =
  ...
  if self.cacheDisp[].isInvalid:
    self.cacheDisp[] = self.upstream.subscribe(
      (v: T) => self.subject.next v,
      (e: ref Exception) => self.subject.error e,
      () => self.subject.complete,
    )
  Disposable.issue:
    tryConsume self.cacheDisp[]
